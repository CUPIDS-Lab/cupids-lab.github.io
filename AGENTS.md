...
`generate-brand.rb` emits SVGs: `favicon.svg`, **`mark.svg`** (transparent mark for inline chrome), `avatar.svg` (profile lockup: folder-with-arrow + wordmark), **`avatar-circle.svg`** (profile picture for round crops), `og.svg` (1200×630 social card), `banner.svg`, **`banner-network.svg`** (1500×500 header), `pattern.svg` (tiling), `background.svg` (1600×900 "folder map"), and a **`folders/`** collection — one recolored file folder per palette hue (e.g. `folders/green.svg`) for list bullets, scatter art, or icons.

Two of those are shaped by where they get used, not by the page:

- **`avatar-circle.svg`** — GitHub, Slack and X mask a profile picture to a circle, which clips the corners of the square `avatar.svg` lockup (wordmark included) and nicks the arrow's point and fletching. This one drops the wordmark and pads the mark so the whole folder-arrow sits inside the circle: `folder_arrow_in_circle` places it by the artwork's *measured* enclosing circle (`ART_CX/ART_CY/ART_R` — a fraction of the seed's frame, measured off the rendered seed; re-measure if the seed art changes), with `MARK_FILL` setting how much of the disc it fills.
- **`banner-network.svg`** — a still frame of the hero's particle-link network, replayed deterministically in Ruby from the same rules as `cupids.js` (folder count, link distance, prevent-overlap separation, the SI spread) with a seeded RNG, then snapshotted once `NET_INFECTED` of the folders are opened. The `NET_*` constants say where it departs from the hero and why: nothing is layered over a banner, so it undims the network, runs the folders a little larger, and bounces them off an inset margin so none is cropped. Change `NET_SEED` to draw a different network.
